...
- **Architecture Decisions (ADRs)** — `decisions/`
  - `ADR-001-cqrs-without-mediatr.md`
  - `ADR-002-transactional-projections-variant-a.md`
  - `ADR-003-global-counter-from-event-position.md`

## How to use

- Start with **DOC-01** and **DOC-02**.
- Use **DOC-03** as the reference for layering, folder structure, CQRS style (no MediatR), and **Asp.Versioning v3 route groups**.
- Use **DOC-04** and **DOC-05** as the source of truth for API contract + persistence model (including optional `UserId` event semantics and operation-scoped idempotency).
- Use **DOC-06**/**DOC-07** for local dev, migrations, and integration testing strategy.
- Use **MANUAL-STEPS.md** to understand when the agent must explicitly ask for human actions (e.g., generating migrations, provisioning Azure secrets).
